...
#include "../util.h"
	...
	battery_perc(const char *bat)
	{
		...
		// Getting the status of the battery to not send 
		// notifications redundantly
		if (esnprintf(status_path, sizeof(status_path),
		              "/sys/class/power_supply/%s/capacity", bat) < 0) {
			return NULL;
		}
		if (pscanf(status_path, "%d", &perc) != 1) {
			return NULL;
		}

		if (perc < 15 && !strcmp("Discharging", state) ){
			notify_init("Battery");
			NotifyNotification *n = notify_notification_new("Akksi", "Mindjárt meghal az akksi, ember!", 0);
			notify_notification_set_urgency(n, NOTIFY_URGENCY_CRITICAL);
			notify_notification_set_timeout(n, 3000);
			notify_notification_show(n, 0);
		}
		else if (perc == 100 && !strcmp("Charging", state) ){
			notify_init("Battery");
			NotifyNotification *n = notify_notification_new("Akksi", "Cuftig van az akksi, kihúzhatod a dugót!", 0);
			notify_notification_set_timeout(n, 3000);
			notify_notification_show(n, 0);
		}
