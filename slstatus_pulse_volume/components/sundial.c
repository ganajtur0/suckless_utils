...
sundial(void) {
    char res_rise[32];
    char res_set[32];
    double sunrise_time = sunrise(lat, lon, gmt, getTime(time(NULL)));
    timestamp_to_str(sunrise_time, "%H:%M", res_rise);
    double sunset_time = sunset(lat, lon, gmt, getTime(time(NULL)));
    timestamp_to_str(sunset_time, "%H:%M", res_set);
    return bprintf("🌅%s 🌇%s", res_rise, res_set);
}
